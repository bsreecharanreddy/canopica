...
-- "widen once per real need" pattern V16/V18/V20/V21/V24/V25 already set
-- -- QC_REVIEW_COMPLETED (Task 5) is that task's own real need, not
-- this one's.
...
alter table audit_event add constraint audit_event_event_type_check
    check (event_type in
        ('APPLICATION_SUBMITTED', 'DETERMINATION_MADE',
         ...
         'FRAUD_FLAG_REVIEWED', 'QC_DISCREPANCY_FLAGGED'));

 ...
 * {@code api/pom.xml}'s extra {@code testResources} entry, so this is the exact same realm config the
...
public abstract class AbstractApiTest extends AbstractPostgresTest {
    ...
    // The whole test (token fetch + the api service's own Spring context) runs in one JVM on the host, both

...
1. Sign in to the [Power BI Service](https://app.powerbi.com) (a free
   ...
   capacity required).
2. Create a new workspace (or reuse an existing dev one), e.g. "Canopica Phase
   1a".
3. **New → Dataflow / Semantic model → Import a file**, and point it at
   `reporting/semantic-model/` -- the Service's TMDL import reads
   `canopica.tmdl` and everything under `tables/` as one model.
   ...
   project's own `definition/` folder structure (`canopica.tmdl` as
...
5. Enter the `canopica_serving` database credentials (`CANOPICA_METABASE_USER`-style
   ...
   `canopica_app` role there instead).

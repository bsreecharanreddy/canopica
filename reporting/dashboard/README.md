...
-- by `ies_data.reporting.provision_metabase`. Run it against a running
...
Credentials and the target URL come from `IES_METABASE_URL`,
`IES_METABASE_USER`, and `IES_METABASE_PASSWORD` (local-dev defaults in
`infra/.env.example` and in `Settings` itself -- see
`data-platform/src/ies_data/config.py`). The script:
...
2. Creates (or finds, by name) a Postgres database connection named
   "IES Serving" pointing at `IES_SERVING_DSN`.
...
## `IES_SERVING_DSN`'s host must resolve for Metabase, not for the caller

`provision_metabase.py` never opens a database connection itself -- it only
sends `IES_SERVING_DSN`'s host/port/dbname/user/password to Metabase's own
...
`IES_SERVING_DSN=...@localhost:5432/...` (correct for a host process
...
`IES_SERVING_DSN=postgresql://ies_app:ies_app@postgres:5432/ies_serving`,
...
`metabase`, so one `IES_SERVING_DSN` value (the `postgres` service name)

...
from canopica_data.synthetic.models import SyntheticHousehold
...
def _fetch_citizen_token(keycloak_url: str, *, transport: httpx.BaseTransport | None) -> str:
    with httpx.Client(transport=transport, timeout=30.0) as client:
        response = client.post(
            f"{keycloak_url}/realms/canopica-citizens/protocol/openid-connect/token",
            data={
                "grant_type": "password",
                "client_id": "test-customer",
                "client_secret": "test-customer-secret",
                "username": "citizen.jordan@canopica.local",
                "password": "CanopicaCitizen123!",
            },
        )

...
def _fetch_worker_token(keycloak_url: str) -> str:
    ...
    `test-worker` client identity/realm-export/ies-workers-realm.json seeds
    for exactly this purpose -- replaces the X-IES-Role header this test
    ...
    response = httpx.post(
        f"{keycloak_url}/realms/ies-workers/protocol/openid-connect/token",
        data={
            "grant_type": "password",
            "client_id": "test-worker",
            "client_secret": "test-worker-secret",
            "username": "worker.sam",
            "password": "IesWorker123!",
        },
        timeout=30.0,
    )

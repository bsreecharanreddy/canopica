...
def test_posts_the_intake_payload_with_the_customer_role_header() -> None:
    ...
    results = post_households(households, "http://test", transport=httpx.MockTransport(handler))

    assert len(results) == 2
    assert len(seen_requests) == 2
    for request in seen_requests:
        assert request.url.path == "/api/applications"
        assert request.headers["X-Canopica-Role"] == "CUSTOMER"
...
def test_raises_on_a_4xx_response() -> None:
    ...
    with pytest.raises(httpx.HTTPStatusError):
        post_households(households, "http://test", transport=httpx.MockTransport(handler))

...
public abstract class AbstractApiTest extends AbstractPostgresTest {
    ...
    static final KeycloakContainer KEYCLOAK =
            new KeycloakContainer("quay.io/keycloak/keycloak:26.4")
                    .withRealmImportFiles("keycloak/ies-workers-realm.json", "keycloak/ies-citizens-realm.json");
    ...
    static void keycloakProperties(DynamicPropertyRegistry registry) {
        registry.add("ies.keycloak.workers-issuer-uri", () -> KEYCLOAK.getAuthServerUrl() + "/realms/ies-workers");
        registry.add("ies.keycloak.workers-jwks-uri",
                () -> KEYCLOAK.getAuthServerUrl() + "/realms/ies-workers/protocol/openid-connect/certs");
        registry.add("ies.keycloak.citizens-issuer-uri", () -> KEYCLOAK.getAuthServerUrl() + "/realms/ies-citizens");
        registry.add("ies.keycloak.citizens-jwks-uri",
                () -> KEYCLOAK.getAuthServerUrl() + "/realms/ies-citizens/protocol/openid-connect/certs");
    }
    ...
    protected static synchronized String workerToken() {
        if (cachedWorkerToken == null) {
            cachedWorkerToken = fetchToken("ies-workers", "test-worker", "test-worker-secret", "worker.sam", "IesWorker123!");
        }
    ...
    protected static synchronized String supervisorToken() {
        if (cachedSupervisorToken == null) {
            cachedSupervisorToken =
                    fetchToken("ies-workers", "test-worker", "test-worker-secret", "supervisor.robin", "IesSupervisor123!");
        }
    ...
    protected static synchronized String citizenToken() {
        if (cachedCitizenToken == null) {
            cachedCitizenToken = fetchToken(
                    "ies-citizens", "test-customer", "test-customer-secret", "citizen.jordan@ies.local", "IesCitizen123!");
        }

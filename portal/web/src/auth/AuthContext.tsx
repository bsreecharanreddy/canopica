...
const IesAuthContext = createContext<IesAuthValue>({ status: 'loading' });

export function useIesAuth(): IesAuthValue {
  return useContext(IesAuthContext);
}
...
function AuthBridge({ realm, onSignedOut, children }: { realm: Realm; onSignedOut: () => void; children: ReactNode }) {
  ...
  // hasn't unmounted yet (that only happens once IesAuthProvider's own realm state resets, a separate,
  ...
  return <IesAuthContext.Provider value={value}>{children}</IesAuthContext.Provider>;
}
...
export function IesAuthProvider({ children }: { children: ReactNode }) {
  ...
  if (realm === null) {
    return (
      <IesAuthContext.Provider value={{ status: 'choosing', chooseRealm }}>
        <RealmChooser onChoose={chooseRealm} />
      </IesAuthContext.Provider>
    );
  }

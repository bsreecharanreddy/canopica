---
name: ies-task-checkpoint
description: Use when completing one task from an IES implementation plan (docs/plans/*.md) — the per-task regression gate CLAUDE.md's non-negotiable testing policy requires before calling a task done.
---
...
`ies-design-decision` skill to record it (STATUS.md's decisions table,

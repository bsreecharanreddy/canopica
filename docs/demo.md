...
```bash
cd data-platform
uv run python -m ies_data.audit.verify_chain --dsn "postgresql://ies_app:ies_app@localhost:5432/ies_operational"
```
...
<http://localhost:3001> (sign in with `admin@ies.local` / `IesAdmin123!`

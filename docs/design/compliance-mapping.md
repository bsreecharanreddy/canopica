...
(`portal/src/main/java/ies/portal/config/SecurityConfig.java`,
`oauth2ResourceServer(...)`), replacing Phase 1a's `X-IES-Role` header
...
backdate a hash. `ies_data.audit.verify_chain` independently re-derives
...
`ies_data.governance.tokenize.detokenize`, a separate, explicit call, not
...
(`data-platform/src/ies_data/config.py`) — a local-dev default,
...
- **Single application role.** `ies_app` is both the portal's and the

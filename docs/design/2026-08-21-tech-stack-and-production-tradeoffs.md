...
| Transformation | dbt, medallion (bronze/silver/gold) | dbt on Databricks/Snowflake/Fabric — or, in older systems, Informatica / DataStage / Ab Initio | **≈** | **Verified for real against Databricks Free Edition** (Phase 5 Task 1), not just claimed: every silver/gold model's SQL ported completely unmodified. What didn't: bronze's own sourcing mechanism (`meta.external_location`, a `dbt-duckdb`-specific feature reading local Delta files via `delta_scan`) has no cross-adapter equivalent — a small seeded bronze slice (`dbt seed`, target-gated so `local` is untouched) stands in for it on Databricks. See `data-platform/databricks-adapter/` and `docs/cloud-demo/`. |
...
| Compute engine | DuckDB (local, in-process) | Spark on Databricks, Fabric, or Synapse | **≈** | A dbt profile swap (`dbt-duckdb` → `dbt-databricks`) — proven for real, not just claimed (Phase 5 Task 1): `dbt-databricks` needs an older `dbt-core` than the DuckDB target's own pin, so the swap runs from an isolated adapter environment, not a shared one. Model SQL is unchanged for every silver/gold model; one shared macro (the gold-layer PII-name guard) needed a `target.type`-dispatched branch, since `SIMILAR TO` is DuckDB/Postgres-standard SQL that Databricks SQL doesn't parse at all (it uses `RLIKE`). |
...
| Layer | Canopica uses | Real production equivalent | Fidelity | What would change |
|---|---|---|---|---|
| Semantic model | Model-as-code (TMDL), version-controlled | Same model, hosted on Fabric/Power BI Premium capacity | **=** | Deploy target only. **Proven for real, not just claimed** (Phase 5 Task 2 Step 5): published against a live Azure Postgres serving database, real numbers (43 determinations, 39.53% eligible, $105.53 average benefit). Two real gaps found: true Fabric capacity is blocked on a fresh tenant by Microsoft's own documented ~90-day new-trial restriction (a Power BI Individual trial was granted instead, already documented above as sufficient for this import); and the TMDL-folder import path needs an on-premises data gateway even for a local file, which is Windows-only — worked around by rebuilding the same three measures directly against the live PostgreSQL connector instead of uploading the TMDL text, not a different model. |
...
| Layer | Canopica uses | Real production equivalent | Fidelity | What would change |
|---|---|---|---|---|
| Runtime | Docker Compose, one host | Kubernetes (AKS/OpenShift), multi-node, multi-AZ — or a container PaaS in commercial Azure, but **not** Container Apps if the target is Azure Government, where it isn't offered (§4.18) | **≈** | Container images are unchanged; orchestration manifests are new. Which orchestrator is available, not which is architecturally preferable, is the binding constraint in Gov cloud. **Proven for real, not just claimed** (Phase 5 Task 2): `infra/azure/`'s reference Terraform was actually applied — every image built for `linux/amd64` (Container Apps rejected the arm64-only images a Mac-native `docker build` produces by default), and five real gaps found only by running it live: Postgres SKU capacity missing in the applied region (fixed with a dedicated, overridable `postgres_location`), zero firewall rules on a "publicly accessible" Postgres server (nothing could reach it, not even sibling Container Apps), an unlisted Postgres extension (Azure's managed service gates `CREATE EXTENSION` in a way self-hosted Postgres doesn't), `ui`'s nginx config hardcoding Docker Compose's own service DNS name instead of Container Apps' `http://<app-name>` convention, and Airflow's metadata database never being initialized (this config models only the long-running processes, not Compose's own one-shot init job). All four apps confirmed genuinely healthy via their real public health endpoints, then torn down the same session. |

...
claim exists today — see the `ies-design-decision` skill if it's picked
...
(`ies_data.config`), Airflow's connection store once Phase 1b builds it,
...
IES today runs in exactly one environment: local Docker Compose. A real

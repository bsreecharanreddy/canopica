...
Retrieved and processed by `data-platform/src/canopica_data/synthetic/fetch_pums.py`, which is
**not run at test time or build time** — the output,
`data-platform/src/canopica_data/synthetic/data/acs_pums_marginals.json`, is committed, so a clone
...
representative than any other), not because Canopica targets it. This is a national data platform
...
`canopica_data.synthetic.generator.generate_households(count, seed=seed)` produces
...
`POST /api/applications` expects. `canopica_data.synthetic.loader.post_households()` submits that

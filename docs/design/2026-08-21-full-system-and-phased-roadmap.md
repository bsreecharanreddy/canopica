...
| AI runtime, public demo | OpenRouter's free-tier (`:free`) models as primary, falling back to a cheap paid OpenRouter model under a $5/mo hard cap, only then failing closed to "unavailable" | Free-tier hosting can't fit even a small quantized LLM at usable latency. A tiered fallback (free → capped paid → unavailable) gives real availability exactly when free-tier limits are most likely to bite — a burst of real traffic — for a fully bounded cost, rather than showing "unavailable" the moment free-tier alone is exhausted. Settled during Phase 2 design, refined after direct discussion; see that design doc §2.7. |
...
| AI capability architecture | Every Phase 2+ AI feature is a fixed, bounded pipeline (retrieve→generate, or NL→tool-call→execute) — never an open-ended autonomous agent that plans and loops over its own tool calls | A fixed pipeline is auditable end to end and easy to keep on the "assists" side of the governing principle; a self-directed multi-step agent is fundamentally harder to bound, log, and review before anything binding happens. Settled during Phase 2 design; see that design doc §2.10. |
| Rule-authoring copilot scope | Proposes new `policy_parameter_set` values (dollar thresholds, deductions) from a changed policy document — not open-ended DMN decision-table restructuring | Reuses the effective-dated, immutable-once-published parameter-version mechanism §3.5 already built; restructuring DMN table *logic* is a fundamentally bigger, higher-risk surface this phase doesn't take on. A real scope narrowing from this doc's own earlier "proposes a DMN decision-table diff" wording; see Phase 2 design doc §2.3. |
| Parameter-set supersession | Publishing a superseding `policy_parameter_set` closes the outgoing one by setting `effective_to` from `null` to a date — the single UPDATE the immutability trigger permits, one-way and only when every other column is bit-identical | Ranges stay non-overlapping, so `findEffectiveOn` keeps erroring on ambiguity instead of silently picking a set. Provably outside the §3.5 reproducibility guarantee: re-derivation resolves by parameter-set *id* and never reads either date column, and `policy_parameter` values stay absolutely immutable. Forced by Phase 2 Task 3, the first code path that publishes a set at runtime; see `2026-08-23-policy-parameter-supersession.md`. |
| Analytics Copilot integration | MetricFlow's semantic layer exposed to the LLM as callable tools over **MCP** (Model Context Protocol); authorization resolved at tool-exposure time, before any query compiles | The 2026-standard mechanism for LLM↔governed-semantic-layer integration, not a bespoke one; a 2026 dbt Labs benchmark found semantic-layer grounding alone closes most of the text-to-SQL accuracy gap. Settled during Phase 2 design; see that design doc §2.4. |
| Analytics Copilot execution engine | MetricFlow queries execute against **DuckDB** (where dbt actually builds gold), not Postgres — `read_only=True` + `enable_external_access=false` + `lock_configuration=true` + file protection is the copilot's real backstop; `canopica_analytics_ro` guards the separate Postgres *serving* layer Metabase/Power BI read, not the copilot | §2.4's original text assumed a Postgres role sat under the copilot without checking where the marts physically live — they're in DuckDB, where a Postgres role isn't on the query path. Self-hosted MetricFlow also has no authorization of its own (row-level security is a dbt-Cloud-only feature), confirming the MCP-tool-exposure gate above has to be the real enforcement point regardless. Settled during Phase 2 Task 4; see `2026-08-24-analytics-semantic-layer-execution-and-authorization.md`. |
| RAG evaluation method | RAGAS metrics (faithfulness, context precision/recall) gated in CI via DeepEval with baseline-relative thresholds, plus a cheap deterministic citation-existence check as a pre-filter | The defined 2026 standard for RAG evaluation; baseline-relative thresholds are the demonstrated fix for "an LLM judge is non-deterministic" without giving up gating on it. Settled during Phase 2 design; see that design doc §2.6. |
| UI design system | "Public Ledger" — a civic-editorial visual language (warm paper base, Canopica Verdigris `#167C6B` reserved for governed/affirmed states, DM Serif Display + DM Sans, a case-file-desk layout with a chain-of-custody spine and decision bar), built on Tailwind + shadcn/ui | Sourced from a Manus design exploration (kept out of this repo — reference only, re-implemented natively), chosen because it makes this system's own evidence-first, human-accountable premise visible rather than decorating over it. Applied to the 5 real pages that exist today; Phase 3/4 screens get it when those phases are real. See `2026-08-27-ui-modernization-public-ledger.md`. |
...
canopica/
  ...
  worker/                   <- pgmq consumer (document intake, correspondence dispatch)
  ai/

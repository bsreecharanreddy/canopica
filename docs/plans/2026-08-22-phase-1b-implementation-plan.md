...
10. **Don't re-create schema that already exists.** Check
    ...
    `portal/src/main/java/canopica/portal/domain/` before writing a migration
...
11. **Every substitution this phase makes gets a tradeoffs-doc row or
    tightened existing row**, per the `canopica-design-decision` skill — most
...
canopica/
  ...
  identity/
    realm-export/canopica-citizens-realm.json   <- Task 1
    realm-export/canopica-workers-realm.json    <- Task 1
    README.md                              <- Task 1
  infra/
    docker-compose.yml                     <- modified: +keycloak, +airflow, +jaeger, +prometheus, +grafana
    airflow/dags/canopica_pipeline_dag.py       <- Task 4
...
| # | Task | Deliverable |
|---|---|---|
| 1 | Keycloak — citizen + worker realms | Real OIDC tokens replace `X-Canopica-Role`; e2e test uses a real token |
...
task as its replacement). Two realms: `canopica-citizens` (customer
self-service accounts) and `canopica-workers` (`WORKER`/`SUPERVISOR`/`ADMIN`
realm roles — the schema for those three values already exists on
`worker.role`, see the design doc §2.1).

**Files:**
- Create: `identity/realm-export/canopica-citizens-realm.json`,
  `canopica-workers-realm.json` — declarative realm config, imported on
...
- Create: `portal/src/main/java/canopica/portal/config/
KeycloakWorkerSyncFilter.java`
- Delete: `portal/src/main/java/canopica/portal/config/
...
**Interfaces:**
- Consumes: Keycloak's `/realms/canopica-workers/protocol/openid-connect/
...
- Produces: every previously-`X-Canopica-Role`-gated endpoint now expects
...
- [ ] **Step 1: Realm exports.** `canopica-workers` realm: client
      `canopica-portal-api` (confidential, for resource-server validation),
      client `canopica-portal-web` (public, PKCE), client `test-worker`
      ...
      `canopica-citizens` realm: simpler, one client, self-registration
...
- [ ] **Step 3: `SecurityConfig` swap.**
      ...
      pointing at the `canopica-workers` realm for worker endpoints and
      `canopica-citizens` for citizen endpoints (two resource-server chains,
...
- [ ] **Step 6: Adapt `test_end_to_end.py` and `docs/demo.md`.** The e2e
      ...
      bearer token from `canopica-workers`' token endpoint using
      `test-worker`'s Direct Access Grant before calling the portal API,
      replacing the `X-Canopica-Role: WORKER` header entirely. Re-run the
...
- Create: `portal/src/main/java/canopica/portal/caseload/
CaseAssignmentService.java`
- Modify: `portal/src/main/java/canopica/portal/domain/Household.java`
  (`isSensitive`, `sensitiveReason`)
- Modify: `portal/src/main/java/canopica/portal/api/
...
- Create: `portal/src/test/java/canopica/portal/caseload/
CaseAssignmentServiceTest.java`
- Modify: `portal/src/test/java/canopica/portal/api/
...
- Modify: `portal/src/main/java/canopica/portal/intake/IntakeService.java`
  (creates a `verification` row per program request, `data_element =
'INCOME'`, `status = 'OUTSTANDING'`)
- Create: `portal/src/main/java/canopica/portal/verification/
MockVerificationService.java`, `VerificationController.java`
- Create: `portal/src/main/java/canopica/portal/domain/
VerificationResponse.java`, `repo/VerificationResponseRepository.java`
- Create: `portal/src/test/java/canopica/portal/verification/
...
**Files:**
- Create: `infra/airflow/dags/canopica_pipeline_dag.py`
- Modify: `infra/docker-compose.yml` (+`airflow` service(s) —
  ...
  already uses for `canopica_operational`/`canopica_serving`)
...
- The DAG's tasks call the exact same entry points `make pipeline`
  already calls (`canopica_data.ingestion.extract`,
  `dbt build` via `BashOperator`,
  `canopica_data.serving.materialize`, `canopica_data.reporting.provision_metabase`
...
- [ ] **Step 2: `canopica_pipeline_dag.py`** — four tasks (`extract`,
...
- [ ] **Step 3: `test_airflow_dag.py`.** DAG-integrity test (imports
      ...
      test canopica_pipeline dag <date>` or the Airflow REST API), asserting
...
**Files:**
- Modify: `data-platform/src/canopica_data/ingestion/extract.py` (extend the
...
- Create: `dbt/canopica_warehouse/models/silver/dim_worker.sql`,
...
- Create: `dbt/canopica_warehouse/models/gold/mart_processing_timeliness.sql`,
...
- Create: `portal/src/main/java/canopica/portal/domain/ResourceRecord.java`,
...
- Modify: `portal/src/main/java/canopica/portal/api/dto/IntakeRequest.java`
  (or wherever household-level fields live)
- Modify: `portal/web/src/pages/IntakePage.tsx` (one new field)
- Modify: `portal/src/test/java/canopica/portal/intake/` tests
- Modify: `docs/STATUS.md`
...
- Create: `data-platform/src/canopica_data/governance/tokenize.py`
- Modify: `dbt/canopica_warehouse/models/silver/dim_person.sql` (stores
  tokens for `first_name`/`last_name`/`date_of_birth` instead of raw
  values)
- Modify: `dbt/canopica_warehouse/models/silver/dim_household.sql` (same for
  address columns)
...
1.0` for a demo)
- Create: `data-platform/src/canopica_data/observability/tracing.py`
- Modify: `data-platform/src/canopica_data/ingestion/extract.py`,

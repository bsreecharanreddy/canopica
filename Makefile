...
seed: ## Generate + load 500 synthetic households into the running stack
	cd data-platform && uv run python -m ies_data.synthetic.cli generate --count 500 --seed 42 --out /tmp/ies-households.jsonl
	cd data-platform && uv run python -m ies_data.synthetic.cli load --input /tmp/ies-households.jsonl --api http://localhost:8080
